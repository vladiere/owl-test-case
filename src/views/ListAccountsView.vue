  ...
  <div class="h-full w-full p-0">
    <table class="w-full text-sm text-center rtl:text-right text-gray-400">
      <thead class="text-xs text-gray-700 uppercase bg-gray-700 text-gray-400">
        <tr>
          <th scope="col" class="px-2 py-3">Firstname</th>
          <th scope="col" class="px-2 py-3">Middlename</th>
          <th scope="col" class="px-2 py-3">Lastname</th>
      ...
      <tbody>
        ...
        >
          ...
          <td class="py-4 justify-center capitalize flex gap-2">
            <router-link
              :to="{ path: '/accounts/update', query: { id: account.id } }"
              class="font-medium text-blue-600 dark:text-blue-500 hover:underline"
              >Edit</router-link
            >
            <button
              class="font-medium text-red-600 dark:text-red-500 hover:underline"
              @click="delete_acc(account.id)"
            >
              Delete
            </button>
          </td>
